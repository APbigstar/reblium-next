...
export async function PUT(req: NextRequest) {
  try {
    ...
    if (amount === undefined) {
      return NextResponse.json({ error: 'Missing amount in request body' }, { status: 400 });
    }

    const getUserCredit = "SELECT amount, premium_status FROM User_Credits WHERE user_id = ? FOR UPDATE";
    ...
    if (userCredits.length === 0) {
      if (premium === 'premium' || premium === 'pro') {
        const insertUserCredit = "INSERT INTO User_Credits (user_id, amount, premium_status) VALUES (?, ?, ?)";
      ...
      const updateUserCredit = "UPDATE User_Credits SET amount = ?, premium_status = ? WHERE user_id = ?";
