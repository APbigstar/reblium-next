  ...
  try {
    ...
    if (!user_plan_id || !plan_id) {
      return NextResponse.json(
        { error: "Missing plan_id or user_plan_id" },
        { status: 400 }
      );
    }
    ...
    if (userPlan.plan_id !== plan.id) {
      ...
      const updateUserPlan = `
        UPDATE User_Plans 
        SET plan_id = ?, expires_at = NULL, created_at = ?
        WHERE id = ?
      `;
      await query(updateUserPlan, [plan.id, user_plan_id, currentTime]);
    } else {
      await updateSubscriptionEndPeriod(userPlan.provider_id, false);
      const updateExpires = `
        UPDATE User_Plans 
        SET expires_at = NULL, created_at = ?
        WHERE id = ?
      `;
      await query(updateExpires, [currentTime, user_plan_id]);
    }
