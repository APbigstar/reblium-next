...
import {
  ...
  Action,
  UserMembershipPlan,
  UserMembershipPlanResult,
  CancelUserMembershipPlanResult,
} from "./types";
...
export function UserMembershipPlansProvider({ children }: Props) {
  ...
  const fetchUserMembershipPlans = (() => {
    const fn = async (
      ...
      try {
        ...
        const response = await fetch("/api/membership/user_plans", {
          method: "GET",
          headers: {
        ...
        }: UserMembershipPlanResult = data;


        if (success) {
          resolve(membershipPlanData.map((item) => item));
  ...
  const loadUserMembershipPlans = useCallback(
    () =>
  ...
  const cancelUserMembershipPlan = useCallback(
    async (user_plan_id: number) => {
      try {
        const token = localStorage.getItem("token");
        if (!token) {
          throw new Error("No token found");
        }
        const response = await fetch("/api/membership/user_plans/cancel", {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${token}`,
          },
          body: JSON.stringify({ user_plan_id }),
        });
        ...
        return { status: response.status, success, result };
      } catch (error) {
  ...
  return (
    <UserMembershipPlansContext.Provider value={memoizedValue}>
      {children}
    </UserMembershipPlansContext.Provider>
  );
}
...
export function useUserMembershipPlansContext() {
  ...
  if (!context)
    throw new Error(
      "useMembershipPlansContext context must be use inside MembershipPlansProvider"
    );
...
export function useActiveUserMembershipPlanList(): UserMembershipPlan[] {
  const userMembershipPlanList: UserMembershipPlan[] =
    useUserMembershipPlanList();

  const filter = useCallback(
    (list: UserMembershipPlan[]): UserMembershipPlan[] =>
      list
  ...
  const [list, setList] = useState<UserMembershipPlan[]>(() =>
    filter(userMembershipPlanList)
  );
...
export function usePossibleSubsriptionUserMembershipPlanList(): UserMembershipPlan[] {
  ...
  const filter = React.useCallback(
    (list: UserMembershipPlan[]): UserMembershipPlan[] => {
      if (!Array.isArray(list)) return []; // Add this check
  ...
  const [list, setList] = React.useState<UserMembershipPlan[]>([]);

  React.useEffect(() => {
    if (Array.isArray(userMembershipPlanList)) {
      setList(filter(userMembershipPlanList));
    }
  }, [filter, userMembershipPlanList]);
...
export function useLastPossibleSubsriptionUserMembershipPlan() {
  ...
  const getLastOne = useCallback(
    (list: UserMembershipPlan[]) => (list.length > 0 ? list[0] : null),
    []
  );
  const [lastOne, setLastOne] = React.useState(() =>
    getLastOne(userMembershipPlanList)
  );
  React.useEffect(() => {
